...
const MusicPlay = () =>{
    ...
    const nextSong = (musicType) =>{
        ...
        const index = currentNatureSound === natureList.length - 1 ? 0 : currentNatureSound + 1
  ...
  const playPause = (song) => {
    ...
    return (
      <div className="audioWrapper">
          {isPlaying ? <PauseCircleFilledIcon onClick={()=> playPause(audio)}/> : <PlayCircleIcon onClick={()=>playPause(audio)}/>}
      </div>
    );
